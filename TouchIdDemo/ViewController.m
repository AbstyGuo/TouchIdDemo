...
//  TouchIdDemo
//
//  Created by guoyf on 2020/12/19.
//
...
- (void)viewDidLoad {
    ...
    [button addTarget:self action:@selector(loadAuthentication) forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)loadAuthentication
{
    ...
    /**
     LAPolicyDeviceOwnerAuthentication              输入多次失败后会自动弹出输入密码解锁页面
     LAPolicyDeviceOwnerAuthenticationWithBiometrics   输入多次失败后将锁定touchid并回调授权失败，只能锁屏后输入密码解锁来重启touchid
     */
